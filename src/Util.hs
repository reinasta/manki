...
import Data.Map.Strict (fromList, keys, filterWithKey)
...
import RIO.List (intersect,headMaybe,sortOn,groupBy,nubBy,insertBy,deleteBy,(\\))
...
isIndexable (Audio _ i _) = True
isIndexable (AudioInsert i _) = True
isIndexable (Cloze i _) = True
isIndexable _ = False
...
getIndicesWith c pred =
  [let Just i = getIndex m in i | m <- flattenAudios c, pred m]
...
addCell (Csv rs) c@(Cell _) | null rs   = Csv [Row [c]]
                            | otherwise =
                                let Row cs = head rs
                                in Csv $ Row (cs ++ [c]) : tail rs
addCell c@(Cell _) (Csv rs) | null rs    = Csv [Row [c]]
                            | otherwise =
                                let Row cs = head rs
                                in Csv $ Row (c:cs) : tail rs
...
deleteEmptyCSV (Csv rs) = 
  let rs' = filter (not . isEmptyCSV) rs
      ...
      delEmptyCells (rw:rws) =
        let Row cs = rw 
        in Row (filter (not . isEmptyCSV) cs) : delEmptyCells rws
...
flattenAudios (Cell (m:ms))
  | isAudio m = let Audio as i ms' = m in (Audio as i [] : ms') ++ flattenAudios (Cell ms)
...
takeCSV (Cell ms) nr nc nm = Cell $ take nm ms
