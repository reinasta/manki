...
runManki = do
  ...
  case (input cmmd, output cmmd) of
    (FileInput inputFileArg, FileOutput outputFileArg) -> do
      -- deal with input
      let ioStrContent filename | null filename = getContents
                                | otherwise     = readFile filename
      str <- ioStrContent inputFileArg
      -- get results
      case parse (csvWith $ rowWith id) "" (strip str) of
        Right csvtree -> do
          ...
          case updatedCsvTree of
            ...
            _ -> error "Expected Csv, got different structure"
        Left err -> error $ "Parse error: " ++ show err
    _ -> error "Expected FileInput and FileOutput"
