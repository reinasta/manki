...
import RIO.List (headMaybe,sortOn,groupBy,nubBy,deleteBy,(\\),nub,find)
import Types
import Util
...
-- allows one to filter audios before copying them; use the 'modifier' filter
copyModAudiosWith :: CSV -> (Markup -> Bool) -> ([Markup] -> [Markup]) -> CSV
copyModAudiosWith c pred modifier =
  let -- extract audios from the first argument (ms1) and copy them in the second (ms2)
      ...
      getAndCopyAudiosWith p ms1 ms2 =
        let audios = modifier $ filter (\m -> p m && isAudio m) ms1
      ...
      copyAudiosInCellWith _ _ cell = cell

  in case c of
       (Cell ms) -> Cell $ getAndCopyAudiosWith pred ms ms
       (Csv rs) -> Csv $ (\r -> copyModAudiosWith r pred modifier) <$> rs
       (Row cs) -> let rowLevelAudios = modifier $ concatMap getAudios cs :: [Markup]
...
copyModAudiosOnceWith c pred modifier =
  ...
  in case c of
       (Cell ms) -> Cell $ getAndCopyAudiosWith pred ms ms
       (Csv rs) -> Csv $ (\r -> copyModAudiosOnceWith r pred modifier) <$> rs
       (Row cs) -> let rowLevelAudios = modifier $ concatMap getAudios cs :: [Markup]
...
placeMultipleAudiosOnceWith _ _ [] = []
...
placeMultipleAudiosOnceInListsWith _ _ [] = []
    ...
    audiosFitForInsertion ads mks =
      let allas = filter isAudio ads
          ...
          firstMatchingAudioIndices =
            [idx | i <- allis, any (audioInsertEq i) allas, Just idx <- [getIndex i]]
          -- collect the first audios matching indices in the first argument (is)
          keepFirstMatching [] _ = []
          keepFirstMatching (i:is) audios =
            let mfound = find (`hasIndex` i) allas
            in case mfound of
                 Nothing -> keepFirstMatching is audios
                 Just found -> found : keepFirstMatching is (deleteBy audioInsertEq found audios)
...
placeAudioWith p a ms =
  ...
  where
    ...
    noInsertDuplicates markups = nubBy eqCustomInsertIndices markups
      where
...
mergeCoindexedAudios ms =
  ...
  where
    ...
    sortGroupWith getMaybeIndex ms' =
      let index m = case getMaybeIndex m of
                      Just n -> n
                      Nothing -> 0 -- Default fallback for Nothing case
    ...
    joinAudios markups =
      let mi = join $ getAudioIndex <$> headMaybe markups
          mms = fmap unwrapAudioElems markups
          attrs = nub $ concatMap getAttrs markups
      in case mi of
